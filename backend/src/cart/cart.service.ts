...
export class CartService {
  ...
  async getCart(userId: string) {
    const items = await this.prisma.cartItem.findMany({
      ...
      include: {
        product: {
          select: {
            id: true, name: true, slug: true, images: true, isActive: true,
          },
        },
        variant: {
          select: {
  ...
  async addItem(userId: string, dto: AddToCartDto) {
    ...
    if (!dto.variantId) throw new BadRequestException('Please select a variant before adding to cart');
  ...
  async updateItem(userId: string, cartItemId: string, dto: UpdateCartItemDto) {
    ...
    if (!item.variant) throw new BadRequestException('Cart item has no variant');
    const effectiveStock = item.variant.stock;
    if (effectiveStock < dto.quantity) throw new BadRequestException('Insufficient stock');
