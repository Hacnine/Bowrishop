...
function DetailRow({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-gray-50 last:border-0">
      <span className="text-xs text-gray-500 w-32 flex-shrink-0 pt-0.5">
        {label}
      </span>
...
function OrderDetailModal({
  ...
  const txId =
    addr?.paymentTransactionId ?? (order as any).paymentTransactionId ?? null;
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-1">
              <DetailRow
                label="Name"
                value={order.user?.name ?? order.guestName}
              />
              <DetailRow
                label="Email"
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-2 space-y-3">
              {order.items?.map((item: any) => {
                const variantSnapshot = item.variantSnapshot as any;
                const variantImages: string[] = variantSnapshot?.images ?? [];
                const productImages: string[] = item.product?.images ?? [];
                const displayImage = variantImages[0] || productImages[0];
                const itemPrice = variantSnapshot?.price ?? item.price;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {variantSnapshot &&
                        (variantSnapshot.color || variantSnapshot.size) && (
                          <div className="text-xs text-gray-500 mt-1 space-y-0.5">
                            {variantSnapshot.color && (
                              <div className="flex items-center gap-1.5">
                                {variantSnapshot.colorHex && (
                                  <span
                                    className="w-3.5 h-3.5 rounded-full border border-gray-300 flex-shrink-0"
                                    style={{
                                      backgroundColor: variantSnapshot.colorHex,
                                    }}
                                  />
                                )}
                                <span>{variantSnapshot.color}</span>
                              </div>
                            )}
                            {variantSnapshot.size && (
                              <div>Size: {variantSnapshot.size}</div>
                            )}
                          </div>
                        )}
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      {variantSnapshot?.price &&
                        Number(variantSnapshot.price) !==
                          Number(item.price) && (
                          <p className="text-xs text-gray-400">
                            {formatCurrency(Number(itemPrice))} each
                          </p>
                        )}
                    </div>
                  </div>
