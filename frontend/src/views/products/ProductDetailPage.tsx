...
function RecentlyViewedSection({
  ...
  const { data: allProducts, isLoading } = useGetRelatedProductsQuery(
    { productId: currentProductId, limit: 20 },
...
function TabSection({
  ...
  const [tab, setTab] = useState<"spec" | "desc" | "reviews">(
    hasSpecs ? "spec" : "desc",
  );
              ...
              <tbody className="divide-y divide-gray-50">
                {Object.entries(
                  ...
                  <tr key={key} className="hover:bg-gray-50">
                    <td className="px-5 py-3.5 font-medium text-gray-700 w-2/5 bg-gray-50/50">
                      {key}
                    </td>
...
const isLoading = isProductLoading && !initialProduct;
  ...
  // Related products — scored by backend
  ...
  const handleThumbnailClick = (img: string, index: number) => {
    ...
    if (hasVariants) {
      const match = product!.variants!.find((v) => v.images?.includes(img));
      if (match) {
        setSelectedColor(match.color ?? null);
        setSelectedSize(match.size ?? null);
        updateUrl(match.color ?? null, match.size ?? null);
        setQty(1);
      }
    }
  };
  ...
  const handleColorSelect = (color: string) => {
    ...
    const stillValid = forColor.some((v) => v.size === selectedSize);
    const nextSize = stillValid ? selectedSize : (forColor[0]?.size ?? null);
  ...
  const handleSizeSelect = (size: string) => {
    ...
    const stillValid = forSize.some((v) => v.color === selectedColor);
    const nextColor = stillValid ? selectedColor : (forSize[0]?.color ?? null);
  ...
  useEffect(() => {
    if (selectedVariant?.images?.[0]) {
      const idx = allProductImages.indexOf(selectedVariant.images[0]);
      if (idx !== -1) setSelectedImage(idx);
    }
          ...
          <div className="aspect-square mb-3 relative">
            {product.videoUrl && showVideo ? (
              ...
              <div className="relative">
                ...
                />
                {product.isPreOrder && (
                  <span className="absolute top-3 left-3 z-10 bg-amber-500 text-white text-xs font-bold px-2.5 py-1 rounded-lg flex items-center gap-1.5">
          ...
          <div className="flex gap-2 flex-wrap">
            {product.videoUrl && (
              <button
                ...
                className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-colors relative flex items-center justify-center bg-gray-900 shrink-0 ${showVideo ? "border-indigo-600" : "border-transparent hover:border-gray-300"}`}
                title="Watch video"
              >
            ...
            {allProductImages.map((img, i) => (
              <button
                key={i}
                onClick={() => {
                  handleThumbnailClick(img, i);
                  setShowVideo(false);
                }}
                className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-colors ${!showVideo && i === selectedImage ? "border-indigo-600" : "border-transparent hover:border-gray-300"}`}
              >
              ...
              <div>
                ...
                {product.preOrderDate && (
                  <p className="text-xs text-amber-600 mt-0.5">
                    Expected:{" "}
                    {new Date(product.preOrderDate).toLocaleDateString(
                      "en-US",
                    ...
                    {uniqueSizes.map((size) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative px-4 py-2 text-sm rounded-xl border-2 font-medium transition-all ${active ? "border-indigo-600 bg-indigo-50 text-indigo-700" : hasStock || product!.isPreOrder ? "border-gray-200 text-gray-700 hover:border-gray-400" : "border-gray-100 text-gray-300"}`}
                        >
                          {size}
                    ...
                    {uniqueColors.map((color) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative w-9 h-9 rounded-full border-2 transition-all ${active ? "border-indigo-600 scale-110 shadow-md" : isAvailable ? "border-gray-300 hover:border-gray-500 hover:scale-105" : "border-gray-200 opacity-25 cursor-not-allowed"}`}
